...
public class CommitOrderResponse implements Serializable {
	...
	public static CommitOrderResponse getclazz(String json) {
		if (json == null) {
			return null;
		}
		try {
			Gson gson = new Gson();
			CommitOrderResponse response = gson.fromJson(json,
	...
	public static List<CommitOrderResponse> getclazz2(String json) {
		if (json == null) {
			return null;
		}
		try {
			Gson gson = new Gson();
			List<CommitOrderResponse> lists = new ArrayList<CommitOrderResponse>();
			lists = gson.fromJson(json,
					new TypeToken<List<CommitOrderResponse>>() {
					}.getType());
			return lists;
		} catch (Exception e) {
			e.printStackTrace();
